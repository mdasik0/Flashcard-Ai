    ...
    <>
      ...
      <div className="flex gap-4 items-center justify-center mt-4">
        ...
        <div ref={dropdownRef} className={`relative ${roboto.className}`}>
          ...
          >
            {selectedDeck ? <FaRegCheckCircle className="text-lg" />:<FaRegCircle className="text-lg" />}
            Deck
            <FaCaretDown
          ...
          {open && (
            <div className="absolute text-center bottom-12 left-0 w-[160px] h-fit rounded-lg p-1.5  bg-[#0E0E0E] border border-[#181818] text-white flex flex-col gap-1.5 items-center">
              ...
              <button onClick={() => setDeck(deckname)} className={`bg-[#181818] hover:bg-[#1d1d1d] ${selectedDeck ? 'text-green-400' : 'text-white'} duration-300 active:scale-95 cursor-pointer text-sm font-medium w-full py-1.5 px-3 rounded flex items-center justify-center gap-2 text-nowrap`}>
                {deckname.length > 15 ? deckname.slice(0, 15) + "..." : deckname}
              </button>
              
              <button className=" bg-[#181818] hover:bg-[#1d1d1d] duration-300 active:scale-95 cursor-pointer text-sm font-medium w-full py-1.5 px-3 rounded flex items-center justify-center gap-2 text-nowrap">
